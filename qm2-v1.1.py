import json
import os
import sys, select, platform, threading
import random
import string
import time
import questionary
...
def show_scores_paginated(scores, page_size=25):
    ...
    for s in scores:
        ...
        total = s.get("total", s.get("ukupno", s.get("total_questions", (correct or 0) + (wrong or 0) + (unanswered or 0))))
    ...
    while True:
        ...
        table = Table(title=f"[bold cyan]📈 Statistics [{start+1}-{end} of {total}]", box=box.SIMPLE)
...
def rename_category(root_dir="categories"):
    ...
    choice = questionary.select("✏️ Choose a category to rename:", 
                               choices=rel_files + ["↩ Back"]).ask()
    if choice == "↩ Back":
        return
...
def delete_category(root_dir="categories"):
    ...
    choice = questionary.select("🗑️ Choose a category to delete:", 
                               choices=rel_files + ["↩ Back"]).ask()
    if choice == "↩ Back":
        return
...
def create_question():
    ...
    if qtype.startswith("1"):
        ...
        console.print("[bold blue]Enter items for the right column (e.g., Programming language, Markup language, OS):")
        for i in range(3):
        ...
        for i in range(3):
            while True:
                ...
                if "-" in par:
                    ...
                    if ll in [chr(97 + j) for j in range(len(left))] and rn.isdigit() and 1 <= int(rn) <= len(right):
                        correct_pairs[ll] = rn
                        break
...
def input_with_timeout(prompt, timeout=60):
    ...
    """
    import platform, sys, select, threading
    ...
    for q in questions:
        ...
        if q["type"] in ("multiple", "truefalse"):
            ...
            while True:
                ...
                if remaining <= 0:
                    if q["type"] == "match":
                        console.print(f"[red]❌ Time is up.")
                    else:
                        console.print(f"[red]❌ Time is up. Correct answer: [bold]{q['correct']}[/]")
                    timeout_count += 1
                    break

                answer = input_with_timeout(f"⏳ {remaining}s - Your choice (letter, x to quit):", remaining)
            ...
            while True:
                ...
                answer = input_with_timeout(f"⏳ {remaining}s - Enter the correct answer (or x to quit):", remaining)
            ...
            for i, (l, r) in enumerate(zip(left, right), start=1):
                table.add_row(f"[bold]{chr(96 + i)})[/bold] {l}", f"[bold]{i}.[/bold] {r}")
            console.print(table)
            ...
            while i < len(left):
                ...
                if remaining <= 0:
                    console.print(f"[red]❌ Time is up. You didn't complete the question.")
                    timeout_count += 1
                    break

                l_item = left[i]
                input_val = input_with_timeout(f"⏳ {remaining}s - Enter the number for pair {chr(97 + i)}) {l_item} (or x to quit):", remaining)
...
def select_category(allow_create=False):
    root_dir = "categories"
    json_rel = get_categories()

    if not json_rel:
        if not allow_create:
            console.print("[red]⚠️ No categories available.")
            return None
        console.print("[yellow]⚠️ No categories. Create a new one.")
        name = Prompt.ask("Enter file name (e.g., geography.json)")
        path = os.path.join(root_dir, name if name.endswith(".json") else f"{name}.json")
        os.makedirs(os.path.dirname(path), exist_ok=True)
        return path
    ...
    if choice == "➕ Create new":
         pass  # moved to 'Manage categories' submenu
    elif choice == "↩ Back":
        return None

    return os.path.join(root_dir, choice)
...
def edit_question(questions):
    ...
    selection = questionary.select("✏️ Choose a question to edit:", choices=choices + ["↩ Back"]).ask()
    ...
    if qtype == "multiple":
        ...
        new_correct = Prompt.ask("New correct answer", default=qdata["correct"], choices=["True", "False"])
...
def edit_question_by_index(questions, index_number):
    ...
    if qtype == "multiple":
        ...
        new_correct = Prompt.ask("New correct answer", default=qdata.get("correct", ""), choices=["True", "False"])
        new_wrong = "False" if new_correct == "True" else "True"
        questions[index] = {
            "type": "truefalse",
            "question": new_question_text,
...
def delete_question_by_index(questions, index_number):
    """Delete a question directly by its ordinal number (1-based)."""
    if not questions:
        console.print("[yellow]⚠️ No questions available to delete.")
        return
    try:
        index = int(index_number) - 1
    except Exception:
    ...
    if index < 0 or index >= len(questions):
        console.print(f"[yellow]⚠️ Number out of range. Allowed 1-{len(questions)}.")
        return
    confirm = questionary.confirm(f"Are you sure you want to delete question #{index+1}?").ask()
    if confirm:
        try:
            deleted = questions.pop(index)
            question_text = deleted.get('question', '') if isinstance(deleted, dict) else str(deleted)
            console.print(f"[red]❌ Deleted question: {question_text[:50]}...")
        except (IndexError, AttributeError) as e:
            console.print(f"[red]⚠️ Error deleting question: {e}")
            return
    else:
        console.print("[yellow]↩ Deletion canceled.")
...
def delete_question(questions):
    ...
    choices = [f"{i+1}. {q['question'][:50]}..." for i, q in enumerate(questions)]
    selection = questionary.select("🗑 Choose a question to delete:", choices=choices + ["↩ Back"]).ask()
    ...
    try:
        ...
        console.print(f"[red]⚠️ Invalid selection format: {e}")
        return
    
    if index < 0 or index >= len(questions):
        console.print(f"[yellow]⚠️ Index out of range. Must be between 1 and {len(questions)}.")
        return
...
def import_remote_file():
    ...
    if is_csv:
        ...
        while True:
            ...
            if base:
                filename = f"{base}{ext}"
                break
            console.print("[yellow]⚠️ Name cannot be empty. Try again.")
        dest_dir = "csv"
    elif is_json:
        ext = ".json"
        while True:
    ...
    while os.path.exists(filepath):
        overwrite = questionary.confirm(f"⚠️ File '{filename}' already exists in '{dest_dir}'. Overwrite?").ask()
        if overwrite:
            break
        ext = ".csv" if is_csv else ".json"
        while True:
            new_base = Prompt.ask("Enter a different file name (without extension)").strip()
            if new_base.lower().endswith(ext):
                new_base = os.path.splitext(new_base)[0]
            new_base = new_base.strip().strip(".")
            if new_base:
                break
            console.print("[yellow]⚠️ Name cannot be empty. Try again.")
...
def show_logo():
    console.print(Panel('''[bold green]🎓 Welcome to
                    •     ┳┳┓      ┓           
            ┏┓  ┓┏  ┓  ┓  ┃┃┃  ┏┓  ┃┏  ┏┓  ┏┓  
            ┗┫  ┗┻  ┗  ┗  ┛ ┗  ┗┻  ┛┗  ┗   ┛   
             ┗                                 
    ...
    while True:
        ...
        if choice.startswith("1"):
            ...
            while True:
                ...
                selection = questionary.select("════════════════════════════════════════════════════════════\n 📂 Questions - choose a category or option:", choices=categories_choices).ask()
                ...
                if selection == "↩ Back":
                    ...
                    while True:
                        opt = questionary.select("🛠️ Manage categories:", choices=[
                            "➕ Create new category",
                            "✏️ Rename category",
                            "🗑️ Delete category",
            ...
            while True:
                ...
                if tools_choice == "↩ Back":
                    ...
                    for dirpath, dirnames, _ in os.walk(categories_root_dir):
                        for d in dirnames:
                            folders.append(os.path.relpath(os.path.join(dirpath, d), categories_root_dir))
                    ...
                    with open(csv_path, newline='', encoding='utf-8') as csvfile:
                        ...
                        for row in reader:
                            ...
                            right = [x.strip() for x in row.get("right", "").split("|") if x.strip()]
                                ...
                                if raw_answers:
                                    for pair in raw_answers.split(","):
                                        if ":" in pair:
                                            ...
                                            if len(parts) == 2:
                                                l, r = parts
                                                answers[l.strip()] = r.strip()
                                                
                                # ✅ Validate pairs
                                if not answers or len(answers) != len(left):
                                    console.print(f"[red]⚠️ Invalid pairs for question: {question_text}")
                                    continue
                                data_list.append({
                                    "type": "match",
                                    "question": question_text,
                    ...
                    console.print(f"[green]✅ CSV converted to JSON and saved as: [bold]{json_path}[/]")
                    ...
                    with open(csv_output_path, mode="w", newline='', encoding="utf-8") as csvfile:
                        ...
                        for p in questions_list:
                            row = {
                                ...
                                "wrong_answers": ",".join(p.get("wrong_answers", [])) if "wrong_answers" in p else "",
                                "left": "|".join(p.get("pairs", {}).get("left", [])) if p.get("type") == "match" else "",
                                "right": "|".join(p.get("pairs", {}).get("right", [])) if p.get("type") == "match" else "",
                                "answers": ",".join([f"{k}:{v}" for k, v in p.get("pairs", {}).get("answers", {}).items()]) if p.get("type") == "match" else ""
                            }
                            writer.writerow(row)

                        console.print(f"[green]✅ JSON successfully exported to CSV: [bold]{csv_output_path}[/]")
                    ...
                    with open(template_path, mode="w", newline="", encoding="utf-8") as f:
                        ...
                        writer.writerow(["multiple", "What is the capital of France?", "Paris", "Rome,Berlin,Madrid", "", "", ""])
                        writer.writerow(["truefalse", "The Sun is a star.", "True", "False", "", "", ""])
                        writer.writerow(["fillin", "The capital of Japan is ______.", "Tokyo", "", "", "", ""])
                        writer.writerow(["match", "Match technologies", "", "", "Python|HTML", "Programming language|Markup language", "a:1,b:2"])
